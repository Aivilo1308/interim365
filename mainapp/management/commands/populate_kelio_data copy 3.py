...
class KelioDataMigrationCorrected:
    ...
    def _migrate_workflow_demo_corrected(self):
        ...
        steps = [
            ...
            ("Employés avec hiérarchie corrigée", self._migrate_employes_via_kelio_api),
    ...
    def _migrate_scoring_demo_corrected(self):
        ...
        steps = [
            ("Structure de base", self._migrate_structure_only),
            ("Employés hiérarchiques", self._migrate_employes_via_kelio_api),
    ...
    def _migrate_test_data_complete_corrected(self):
        ...
        steps = [
            ...
            ("Employés hiérarchiques test", self._migrate_employes_via_kelio_api),
    ...
    def _migrate_employes_via_kelio_api(self):
        """
        ✅ Importe les employés via l'API SOAP de Kelio avec gestion des erreurs complète
        """
        ...
        self._write("🔄 Importation des employés via API SOAP Kelio...")
        
        try:
            # Import du service de synchronisation Kelio
            from mainapp.services.kelio_api_simplifie_modif import get_kelio_sync_service
            
            if not self.kelio_config:
                error_msg = "❌ Configuration Kelio non disponible"
                logger.error(error_msg)
                self._write(error_msg, self.style.ERROR if self.style else None)
                return
            
            # Initialiser le service Kelio
            sync_service = get_kelio_sync_service(self.kelio_config)
            
            # Test de connexion avant importation
            self._write("🔍 Test de connexion à l'API Kelio...")
            test_results = sync_service.test_connexion_complete()
            
            if not test_results.get('statut_global', False):
                error_msg = f"❌ Échec de connexion à Kelio: {test_results.get('message', 'Erreur inconnue')}"
                logger.error(error_msg)
                self._write(error_msg, self.style.ERROR if self.style else None)
                
                # Détails des services en échec
                services_echec = []
                for service_name, service_info in test_results.get('services', {}).items():
                    if service_info.get('statut') != 'OK':
                        services_echec.append(f"{service_name}: {service_info.get('message', 'Erreur')}")
                
                if services_echec:
                    self._write("📋 Services en échec:")
                    for service_echec in services_echec:
                        self._write(f"  • {service_echec}")
                
                return
            
            self._write("✅ Connexion à Kelio établie avec succès")
            
            # Récupération des données de référence nécessaires
            ...
            if not departements:
                self._write("⚠️ Aucun département disponible, création d'un département par défaut")
                # Créer un département par défaut si nécessaire
                Departement = self.models['Departement']
                dept_defaut, created = Departement.objects.get_or_create(
                    code='DEFAULT',
                    defaults={
                        'nom': 'Département par défaut',
                        'description': 'Département créé automatiquement lors de l\'importation Kelio',
                        'actif': True
                    }
                )
                departements = [dept_defaut]
                self.created_objects['departements'] = departements
            
            if not sites:
                self._write("⚠️ Aucun site disponible, création d'un site par défaut")
                Site = self.models['Site']
                site_defaut, created = Site.objects.get_or_create(
                    nom='Site par défaut',
                    defaults={
                ...
                sites = [site_defaut]
                self.created_objects['sites'] = sites
            
            # Récupération des employés depuis Kelio
            self._write("📥 Récupération des employés depuis Kelio...")
            
            try:
                # Appel à l'API Kelio pour récupérer les employés
                employees_data = sync_service.get_employees()
                
                if not employees_data:
                    error_msg = "❌ Aucune donnée d'employé récupérée depuis Kelio"
                    logger.error(error_msg)
                    self._write(error_msg, self.style.ERROR if self.style else None)
                    return
                
                total_employees = len(employees_data)
                self._write(f"📊 {total_employees} employé(s) récupéré(s) depuis Kelio")
                
            except Exception as api_error:
                error_msg = f"❌ Erreur lors de la récupération des données Kelio: {str(api_error)}"
                logger.error(error_msg)
                self._write(error_msg, self.style.ERROR if self.style else None)
                return
            
            # Traitement et importation des employés
            employees_imported = 0
            employees_updated = 0
            employees_errors = 0
            error_details = []
            
            # Mapping des types de profils Kelio vers les types Django
            KELIO_TO_DJANGO_PROFILE_MAPPING = {
                ...
                'HR': 'RH',
                'ADMIN': 'ADMIN',
                'SUPER_ADMIN': 'ADMIN'  # Les super admins Kelio deviennent ADMIN Django
            }
            
            # Mapping des statuts Kelio vers Django
            KELIO_TO_DJANGO_STATUS_MAPPING = {
                'ACTIVE': 'ACTIF',
                'INACTIVE': 'INACTIF',
                'SUSPENDED': 'SUSPENDU',
                'TERMINATED': 'INACTIF'
            ...
            for employee_data in employees_data:
                try:
                    with transaction.atomic():
                        ...
                        kelio_employee_key = employee_data.get('employee_key') or employee_data.get('id')
                        if not kelio_employee_key:
                            error_details.append("Employé sans clé Kelio valide")
                            employees_errors += 1
                            continue
                        
                        # Données utilisateur Django
                        username = employee_data.get('username') or employee_data.get('badge_code', f'emp_{kelio_employee_key}')
                        first_name = employee_data.get('first_name', '').strip()
                        last_name = employee_data.get('last_name', '').strip()
                        email = employee_data.get('email', f'{username}@entreprise.ci')
                        
                        # Validation des données obligatoires
                        if not first_name or not last_name:
                            error_details.append(f"Employé {kelio_employee_key}: Nom/prénom manquant")
                            employees_errors += 1
                            continue
                        
                        # Recherche ou création de l'utilisateur Django
                        user_updated = False
                        try:
                            ...
                            kelio_profile_type = employee_data.get('profile_type', 'EMPLOYEE')
                            if kelio_profile_type == 'SUPER_ADMIN' or employee_data.get('is_super_admin', False):
                        ...
                        # Recherche ou création du profil utilisateur
                        matricule = employee_data.get('matricule') or employee_data.get('badge_code', f'MAT{kelio_employee_key}')
                        
                        # Mapping du type de profil
                        kelio_profile_type = employee_data.get('profile_type', 'EMPLOYEE')
                        django_profile_type = KELIO_TO_DJANGO_PROFILE_MAPPING.get(kelio_profile_type, 'UTILISATEUR')
                        
                        # Mapping du statut
                        kelio_status = employee_data.get('status', 'ACTIVE')
                        django_status = KELIO_TO_DJANGO_STATUS_MAPPING.get(kelio_status, 'ACTIF')
                        
                        # Assignation du département
                        departement_kelio_key = employee_data.get('department_key')
                        departement = None
                        if departement_kelio_key:
                            departement = next(
                                (d for d in departements if getattr(d, 'kelio_department_key', None) == departement_kelio_key),
                                None
                            )
                        if not departement:
                            departement = departements[0]  # Département par défaut
                        
                        # Assignation du site
                        site_kelio_key = employee_data.get('site_key')
                        site = None
                        if site_kelio_key:
                            site = next(
                                (s for s in sites if getattr(s, 'kelio_site_key', None) == site_kelio_key),
                                None
                            )
                        if not site:
                            site = sites[0]  # Site par défaut
                        
                        # Assignation du poste
                        poste = None
                        poste_kelio_key = employee_data.get('job_key')
                        if poste_kelio_key and postes:
                            poste = next(
                                (p for p in postes if getattr(p, 'kelio_job_key', None) == poste_kelio_key),
                                None
                            )
                        ...
                        profil_data = {
                            'matricule': matricule,
                            'type_profil': django_profile_type,
                            'statut_employe': django_status,
                            'departement': departement,
                            'site': site,
                            'poste': poste,
                            'actif': employee_data.get('is_active', True),
                            'date_embauche': employee_data.get('hire_date'),
                            'date_sortie': employee_data.get('termination_date')
                        }
                        ...
                        # Création ou mise à jour du profil Kelio
                        kelio_profil_data = {
                            'kelio_employee_key': kelio_employee_key,
                            'kelio_badge_code': employee_data.get('badge_code', ''),
                            'kelio_department_name': employee_data.get('department_name', ''),
                            'kelio_job_title': employee_data.get('job_title', ''),
                            'derniere_synchronisation': timezone.now()
                        }
                        
                        kelio_profil, kelio_created = ProfilUtilisateurKelio.objects.get_or_create(
                            profil=profil,
                            defaults=kelio_profil_data
                        )
                        
                        if not kelio_created:
                            for key, value in kelio_profil_data.items():
                                setattr(kelio_profil, key, value)
                            kelio_profil.save()
                        
                        # Création ou mise à jour du profil étendu
                        extended_data = {
                            'telephone': employee_data.get('phone', ''),
                            'telephone_mobile': employee_data.get('mobile', ''),
                            'adresse': employee_data.get('address', ''),
                            'ville': employee_data.get('city', ''),
                            'code_postal': employee_data.get('postal_code', ''),
                            'disponible_interim': employee_data.get('available_for_interim', True),
                            'rayon_deplacement_km': employee_data.get('travel_radius_km', 50),
                            'langues_parlees': employee_data.get('languages', []),
                            'certifications': employee_data.get('certifications', [])
                        }
                        
                        extended_profil, extended_created = ProfilUtilisateurExtended.objects.get_or_create(
                            profil=profil,
                            defaults=extended_data
                        )
                        
                        if not extended_created:
                            for key, value in extended_data.items():
                                if value is not None:
                                    setattr(extended_profil, key, value)
                            extended_profil.save()
                        
                        # Ajout à la liste des objets créés
                        if profil not in self.created_objects.get('employes', []):
                            self.created_objects.setdefault('employes', []).append(profil)
                        
                        # Mise à jour des statistiques
                        if profil_created or not user_updated:
                            employees_imported += 1
                            self._update_stats('ProfilUtilisateur', True)
                        else:
                            employees_updated += 1
                            self._update_stats('ProfilUtilisateur', False)
                        
                        # Log périodique du progrès
                        if (employees_imported + employees_updated) % 10 == 0:
                            self._write(f"  📊 Traité: {employees_imported + employees_updated}/{total_employees} employés")
                
                except Exception as employee_error:
                    employees_errors += 1
                    error_msg = f"Erreur employé {employee_data.get('employee_key', 'inconnu')}: {str(employee_error)}"
                    error_details.append(error_msg)
                    logger.error(f"Erreur importation employé: {employee_error}")
            ...
            self._write("")
            self._write("📊 RÉSUMÉ DE L'IMPORTATION KELIO:")
            self._write("=" * 50)
            
            if employees_imported > 0 or employees_updated > 0:
                success_msg = f"✅ Importation réussie!"
                self._write(success_msg, self.style.SUCCESS if self.style else None)
                self._write(f"  📥 Employés importés: {employees_imported}")
                self._write(f"  🔄 Employés mis à jour: {employees_updated}")
                self._write(f"  📊 Total traité: {employees_imported + employees_updated}")
                
                # Affichage de la répartition hiérarchique
                if self.created_objects.get('employes'):
                    self._write("")
                    self._write("👥 Répartition hiérarchique importée:")
                    hierarchy_count = {}
                ...
                logger.info(f"✅ Importation Kelio réussie: {employees_imported} créés, {employees_updated} mis à jour")
            
            else:
                error_msg = "❌ Aucun employé n'a pu être importé depuis Kelio"
                self._write(error_msg, self.style.ERROR if self.style else None)
                logger.error(error_msg)
            
            # Affichage des erreurs
            if employees_errors > 0:
                self._write(f"⚠️ Erreurs rencontrées: {employees_errors}")
                if error_details:
                    self._write("📋 Détails des erreurs:")
                    for error in error_details[:10]:  # Afficher max 10 erreurs
                        self._write(f"  • {error}")
                    if len(error_details) > 10:
                        self._write(f"  ... et {len(error_details) - 10} autres erreurs")
                
                logger.warning(f"⚠️ {employees_errors} erreurs lors de l'importation Kelio")
            
            self._write("=" * 50)
            
            # Mise à jour du cache si l'importation s'est bien passée
            if employees_imported > 0 or employees_updated > 0:
                try:
                    self._write("🔄 Mise à jour du cache Kelio...")
                    # Optionnel: mettre en cache les données importées
                    # sync_service.update_cache('employees', employees_data)
                    self._write("✅ Cache mis à jour")
                except Exception as cache_error:
                    logger.warning(f"⚠️ Erreur mise à jour cache: {cache_error}")
                    self._write("⚠️ Erreur lors de la mise à jour du cache")
            
        except ImportError as import_error:
            error_msg = f"❌ Service Kelio non disponible: {str(import_error)}"
            logger.error(error_msg)
            self._write(error_msg, self.style.ERROR if self.style else None)
            self._write("💡 Vérifiez que le module 'mainapp.services.kelio_api_simplifie' est disponible")
            
        except Exception as general_error:
            error_msg = f"❌ Erreur générale lors de l'importation Kelio: {str(general_error)}"
            logger.error(error_msg)
            self._write(error_msg, self.style.ERROR if self.style else None)
            self._write("💡 Consultez les logs pour plus de détails")

    def _migrate_employes_corrected(self):
        """
        ✅ Migration des employés avec hiérarchie corrigée - Utilise l'API SOAP Kelio
        """
        self._migrate_employes_via_kelio_api()


    def _create_test_employes_corrected_fallback(self):
        """
        ✅ Version de fallback avec données de test si l'API Kelio n'est pas disponible
        Cette méthode sera utilisée uniquement en mode test ou si l'API échoue
    ...
    def _migrate_employees_only(self):
        ...
        steps = [
            ("Employés", self._migrate_employes_via_kelio_api),
