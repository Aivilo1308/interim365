...
def _generer_explications_score(employe, demande, score_detail):
    ...
    try:
        ...
        if employe.poste and demande.poste:
            if employe.poste.departement == demande.poste.departement:
                ...
                explications['explications_criteres']['similarite_poste'] = [
                    f"Employé du département {employe.poste.departement.nom}",
                    f"Poste demandé dans le département {demande.poste.departement.nom}",
                    "⚠ Changement de département à envisager"
                ]
        ...
        if competences_employe > 0:
            ...
            explications['points_faibles'].append("Faible compétence")
            explications['explications_criteres']['competences'] = [
                "Peu ou pas de compétence enregistrée dans le système",
...
def api_workflow_status(request, demande_id):
    """
    API endpoint pour récupérer le statut workflow d'une demande
    ...
    try:
        ...
        # Utiliser la méthode du modèle pour obtenir le statut workflow
...
def _get_actions_disponibles(profil_utilisateur, demande):
    """
    Détermine les actions disponibles pour l'utilisateur sur cette demande
    """
    actions = []
    ...
    try:
        ...
        logger.warning(f"Erreur détermination actions disponibles: {e}")
...
def _get_prochaines_echeances(demande):
    """
    Calcule les prochaines échéances importantes
    """
    echeances = []
    ...
    try:
        ...
        # Échéance de validation selon l'urgence
        ...
        try:
            ...
            for reponse in reponses_attente:
                echeances.append({
                    ...
                    'label': f'Réponse {reponse.candidat.nom_complet}',
        ...
        logger.warning(f"Erreur calcul échéances: {e}")
    
    return sorted(echeances, key=lambda x: x['date'])[:5]  # Limiter à 5 échéances


def _format_temps_restant(timedelta_obj):
    """
    Formate un objet timedelta en chaîne lisible
    """
    try:
